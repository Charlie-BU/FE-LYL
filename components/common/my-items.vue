		...
		<view class="zp-item" v-for="(item, index) in datas" :key="index" @click="itemClick(item, 1, index)">
			...
			<view class="zp-item-bot">
				<view class="left">
					...
					<view class="name-title" v-if="item.type == 1">{{ item.user.qy_name }}<text v-if="item.star_as_elite"
							class="score">{{ item.star_as_elite[1] }} 分</text></view>
					<view class="name-title" v-else>{{ item.user.user_name }}<text v-if="item.star_as_elite"
							class="score">{{ item.star_as_elite[1] }} 分</text></view>
				</view>
				<view class="right">
			...
			<view class="zp-item-bot-btns" v-if="showBot">
				...
				<view class="left" v-if="item.status == 1">
					<image src="https://liyilian.oss-cn-hangzhou.aliyuncs.com/static/index/check-pass-theme.png"
						mode="widthFix" class="left-icon" />
				...
				<view class="left" v-if="item.status == 4">
					<image src="https://liyilian.oss-cn-hangzhou.aliyuncs.com/static/index/yitingyong.png"
						mode="widthFix" class="left-icon" />
				...
				<view class="right">
					<block v-if="item.status == -1">
						<button size="mini" class="theme-btn" @click.stop="itemClick(item, 2, index)">
							<image src="https://liyilian.oss-cn-hangzhou.aliyuncs.com/static/index/check-pass-white.png"
								mode="widthFix" class="btn-icon" />
							<text>通过</text>
						</button>
						<button size="mini" class="red-btn" @click.stop="itemClick(item, 3, index)">
							<image src="https://liyilian.oss-cn-hangzhou.aliyuncs.com/static/index/bohui.png"
								mode="widthFix" class="btn-icon" />
							<text>驳回</text>
						</button>
					</block>
					...
					<block v-if="item.status == 1 || item.status == 4">
						<button size="mini" class="qy-btn" @click.stop="itemClick(item, 4, index)">
							<image src="https://liyilian.oss-cn-hangzhou.aliyuncs.com/static/index/qy-icon.png"
								mode="widthFix" class="btn-icon" />
							<text>启用</text>
						</button>
					</block>
