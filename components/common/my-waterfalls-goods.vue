<template>
	<view>
		<custom-waterfalls-flow ref="waterfallsFlowRef" :value="list" :column="column" imageKey="goods_img"
			:columnSpace="1.5" :listStyle="{ 'box-shadow': '0px 0px 8rpx rgba(0,0,0,0.2)' }" @imageClick="wapperClick"
			...
			<view class="goods-item" v-for="(item, index) in list" :key="index" slot="slot{{index}}">
				...
				<view class="goods-item-bot">
					...
					<image src="https://liyilian.oss-cn-hangzhou.aliyuncs.com/static/common/join-cart.png"
						@click.stop="add_cart(item.goods_id)" v-if="showCart" class="join_cart" mode="widthFix" />
				</view>
			</view>
			<!-- #endif -->
			<!-- #ifndef MP-WEIXIN -->
