			...
			success:function(g){
				if(g.data.status&&"1"==g.data.status){
					...
					if(g.pois&&g.pois[0]){
						...
						l&&(m=parseFloat(l.split(",")[0]),
						n=parseFloat(l.split(",")[1]))}h.provice&&f.push(h.provice);
					...
					else a.fail({errCode:g.data.infocode,errMsg:g.data.info})},fail:function(g){a.fail({errCode:"0",errMsg:g.errMsg||""})}})}var c=this;a.location?b(a.location):c.getWxLocation(a,function(d){b(d)})};
