			...
			<scroll-view scroll-y="true" class="simple-address__multiple" v-if="showAddress">
				<view class="popup-con-t">
					<view class="popup-content" v-for="(item, index) in address" :key="index">
						...
						<image class="popup-content-image"
							src="https://liyilian.oss-cn-hangzhou.aliyuncs.com/static/common/delete.png"
							@click="addressclick(index)"></image>
					</view>
				</view>
			</scroll-view>
			<view class="simple-address__box">
				<picker-view indicator-style="height: 70rpx;" class="simple-address-view" :value="pickerValue"
					@change="pickerChange" v-if="customArr.length > 0">
					<picker-view-column>
						<!-- #ifndef APP-NVUE -->
				...
				<picker-view indicator-style="height: 70rpx;" class="simple-address-view" :value="pickerValue"
					...
					<picker-view-column v-show="level >= 1">
						<!-- #ifndef APP-NVUE -->
						<view class="picker-item" :style="{ fontSize: fontSize }" v-for="(item, index) in cate1List"
							:key="index">{{ item[customKey] }}</view>
						<!-- #endif -->
						<!-- #ifdef APP-NVUE -->
						<text class="picker-item" :style="{ fontSize: fontSize }" v-for="(item, index) in cate1List"
							:key="index">{{ item[customKey] }}</text>
						<!-- #endif -->
					</picker-view-column>
					...
					<picker-view-column v-show="level >= 4">
						<!-- #ifndef APP-NVUE -->
						<view class="picker-item" @click="clickCate4(item)" :style="{ fontSize: fontSize }"
							v-for="(item, index) in cate4List" :key="index">{{ item[customKey] }}</view>
						<!-- #endif -->
						<!-- #ifdef APP-NVUE -->
						<text class="picker-item" @click="clickCate4(item)" :style="{ fontSize: fontSize }"
							v-for="(item, index) in cate4List" :key="index">{{ item[customKey] }}</text>
						<!-- #endif -->
					</picker-view-column>
				</picker-view>
			</view>
