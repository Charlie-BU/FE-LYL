<template>
	<view class="container">
					...
					<block v-for="(item, index) in lists">
						...
						<block v-if="is_self(item)">
							...
							<view class="item item-time" v-if="item.type == 'refuse_wx_phone'"
								...
								<text class="grey-text">{{ item.payload.type == 1 ? '您拒绝了与对方交换电话' :
									'您拒绝了对方与您交换微信' }}</text>
							</view>
							<view class="item item-time" v-if="item.type == 'agree_wx_phone'"
								:id="`item-${item.messageId}`" :key="item.messageId">
								<text class="grey-text">{{ item.payload.type == 1 ? '您同意了与对方交换电话' :
									'您同意了对方与您交换微信' }}</text>
							</view>
						...
						<block v-else>
							...
							<view class="item item-time" v-if="item.type == 'refuse_wx_phone'"
								:id="`item-${item.messageId}`" :key="item.messageId">
								<text class="grey-text">{{ item.payload.type == 1 ? '对方拒绝了与您交换电话' :
									'对方拒绝了与您交换微信' }}</text>
							</view>
		...
		<u-popup :show="showEvaluateItemsModal" @close="showEvaluateItemsModal = false" mode="center"
			:customStyle="customAlertStyle" bgColor="transparent" :overlayOpacity="0.4">
			<view class="cooperate-popup">
				<view class="cooperate-title">
					<text>合作评价</text>
