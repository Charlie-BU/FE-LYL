		...
		<block v-else>
			<view class="user-top">
				...
				<view class="my-top" v-if="user_id > 0">
					...
					<view class="center">
						...
						<view v-if="star" class="center-bot">
							<text>{{ star[0] }} {{ star[1] }}分</text>
						</view>
					</view>
					<view class="right">
						<view class="right-item" @click="toNext('/pages/my/user-personal')">
							<image src="@/static/my/bianji.png" mode="widthFix"></image>
							<text>编辑信息</text>
						</view>
				...
				<view class="user-top-items" v-else>
					<view class="user-top-item" @click="goNext('/my/link-up')">
						<text class="num-text">{{ identity == 1 ? (user.chat_xm_count || 0) : (user.chat_jl_count ||
							0) }}</text>
						<text class="sub-text">沟通过</text>
					</view>
					<view class="user-top-item" @click="goNext('/pages/my/user-collect')">
						<text class="num-text">{{ identity == 1 ? (user.collect_xm_count || 0) : (user.collect_jl_count
							|| 0) }}</text>
				...
				<view class="user-top-bot">
					...
					<button size="mini" @click="goNext('/my/my-resume')" v-if="identity == 1">{{ temp_user.btn_title
					}}</button>
					<view class="switch" v-if="identity == 3">
						<u-switch asyncChange v-model="online" :inactiveValue="0" :activeValue="1" size="20"
							:loading="switchLoad" :disabled="switchLoad" activeColor="#03AAAB" @change="switchChange" />
					</view>
					<button size="mini" @click="goNext('/pages/talents/publish')"
						v-if="identity == 2 && user.item_count < 10">{{ temp_user.btn_title }}</button>
				</view>
				<view v-if="identity === 1" class="user-top-bot">
					...
					<view class="switch">
						<u-switch asyncChange v-model="talentOnline" :inactiveValue="0" :activeValue="1" size="20"
							:loading="switchLoad" :disabled="switchLoad" activeColor="#03AAAB"
							@change="talentOnlineChange" />
					</view>
				</view>
				<!-- <view class="user-top-bot">
					<image src='/static/my/zaixian.png' mode="widthFix"/>
				...
				<!-- <view class="user-top-bot" v-if="identity != 3">
					...
					<button size="mini" v-if="user"
						@click="goNext('/pages/my/servicepay?identity='+identity)">去查看</button>
			...
			<view class="order-entry" v-if="identity === 1 || identity === 2">
				<view class="order-buttons">
					<view class="order-button" @click="goNext('/pages/service/orders?status=all')">
						...
						<view style="margin-bottom: 7px; margin-top: 10px;">
							<u-icon name="file-text" color="#1abfc0" size="42"></u-icon>
						</view>
						<text>全部订单</text>
					</view>
			...
			<view class="info">
				...
				<view class="tabs-info" v-if="false">
					<u-tabs lineColor="#15B3B4" :list="list2" :scrollable="false" :activeStyle="{
						color: '#000000',
						fontWeight: 'bold',
						transform: 'scale(1.05)',
						fontSize: '28rpx'
						...
						<view class="jl-item-container">
							<view class="jl-item" v-if="Object.keys(resume).length > 0">
								...
								<view class="jl-item-tab">
									<view class="tab-item" v-for="(item, index) in resume.arr" :key="index">{{ item }}
									</view>
								</view>
								<view class="jl-item-bot" :class="{ 'column': resume.status == 2 }">
									...
									<view class="right">
										<button size="mini" class="theme-btn" @click="goNext('/my/my-resume')">
											<image src="@/static/my/bianji-btn.png" mode="widthFix"
												class="right-icon" />
											<text>编辑</text>
										</button>
										<button size="mini" class="ty-btn" @click.stop="tyClick()"
...
export default {
	...
	methods: {
		...
		getTalentOnline() {
			...
			fetch_data("POST", "get_talent_online", {
		...
		xmItemClick(res) {
			...
			switch (res.type) {
				...
				case 5:
					...
					break;
				case 6:
					_this.toNext(`/pages/index/detail?id=${res.item.id}`)
					break;
				default:
					break;
			}
