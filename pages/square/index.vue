<template>
    <div class="app">
        <div style="display: flex">
            <view style="width: 75%; margin-left: 50rpx">
                <u-tabs
                    lineColor="#15B3B4"
                    :list="tabbar_list"
                    :scrollable="true"
            ...
            <view>
                <button class="post-button" @click="send_post">+</button>
            </view>
        </div>
        <!-- <div style="display: flex;">
                ...
                <div class="boxList flex">
                    ...
                    <div @click="go_to_detail(post.id, post.liked)" class="right">
                        ...
                        <div @click.stop="" v-if="post.post_image" class="post-content">
                            <div v-if="post.post_image.image_length >= 1 && post.post_image.image_length <= 3" class="image-grid" style="height: 200rpx">
                                <div v-for="(image, i) in post.post_image" :key="i" class="image-item">
                                    <img
                                        v-if="image && image.length >= 6"
                                        class="post-images"
                                        :src="image"
                                        mode="aspectFill"
                                        @click.stop="image_operation(post.post_image, image)"
                                    />
                                </div>
                            </div>
                        ...
                        <div style="display: flex; justify-content: space-between">
                            <span class="post-type-bar"># {{ postTypeName(post.type) }} #</span>
                            <div style="display: flex; align-items: center">
                                <div style="display: flex; align-items: center; margin-right: 30rpx" @click.stop="like_post_or_cancel(post)">
                                ...
                                <div style="display: flex; align-items: center" @click.stop="send_comment(post)">
        ...
        <div v-if="showPostModal" class="modal-overlay">
            <div class="modal">
                ...
                <div class="form-group">
                    ...
                    <input id="post-title" v-model="new_post_title" type="text" placeholder="请输入标题" class="input-field" />
                </div>

                <!-- 内容 -->
                <div class="form-group">
                    <label for="post-content">内容</label>
                    <textarea id="post-content" v-model="new_post_content" placeholder="说点什么吧..." class="textarea-field"></textarea>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="category-options">
                        <label v-for="(category, index) in categories" :key="index">
                            <div v-if="category.id === new_post_type" class="category-option-selected" @click="select_type(category.id)" :id="'category-' + category.id">
                                <span class="category-label">{{ category.name }}</span>
                            </div>
                            <div v-else class="category-option-unselected" @click="select_type(category.id)" :id="'category-' + category.id">
                                <span class="category-label">{{ category.name }}</span>
                            </div>
                        </label>
                    </div>
        ...
        upload_images() {
            ...
            const uploadPromises = this.new_post_images.map((image_path) => {
                return new Promise((resolve, reject) => {
                    upload_file("upload_image_to_OSS", image_path, "image", "application", (res) => {
                        let response;
                        try {
