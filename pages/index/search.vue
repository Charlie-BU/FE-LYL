<template>
	<view class="container">
		<view class="search-top">
			<u-search bgColor="#EFEFEF" placeholder="搜索职位/公司/内容" @search="search" @custom="search" :actionStyle="{
				'color': '#02AAAB'
			...
			<view class="search-container">
				<view class="search-title">
					...
					<view class="search-title-right" v-if="search_list.length > 0" @tap='qingkong()'>
						<image src="https://liyilian.oss-cn-hangzhou.aliyuncs.com/static/common/del-grey-icon.png"
							class="search-title-right-icon" mode="widthFix" />
					</view>
				</view>
				...
				<view class="search-title" :style="{ 'marginTop': search_list.length > 0 ? '0' : '20rpx' }"
					v-if="false">
					<text class="search-title-text">热门搜索</text>
				</view>
				<view class="items" v-if="hot_keywords.length > 0 && false">
					<view class="item" v-for="(item, index) in hot_keywords" :key="index"
						@click="toNext(`/pages/index/search-goods?keyword=${item}`)">
						<image src="https://liyilian.oss-cn-hangzhou.aliyuncs.com/static/index/hot.png" mode="widthFix"
							class="hot-img" />
						<text>{{ item }}</text>
					</view>
				</view>
