<template>
	<view class="container">
			...
			<view class="form-info">
				<view class="form-row">
					...
					<view class="row-input">
						<u-input placeholder="请输入项目标题" border="none" type="text" placeholderClass="placeholder28"
							v-model="form.title" clearable />
					</view>
				</view>
				<view class="form-row">
					<view class="row-left">
				...
				<view class="form-row">
					...
					<view class="row-input" @click="goNext('talents',3)">
						<text
							:class="{'text-c3':other.talents == ''}">{{ other.talents ? other.talents : '请选择需求技能'}}</text>
					</view>
				...
				<view class="form-row">
					...
					<view class="row-input">
						<text @click="showStartDate = true"
							:class="{'text-c3':form.hz_start_time == ''}">{{ form.hz_start_time ? form.hz_start_time : '选择开始时间'}}</text>
						<text class="heng-text">—</text>
						<text @click="showEndDate = true"
							:class="{'text-c3':form.hz_end_time == ''}">{{ form.hz_end_time ? form.hz_end_time : '选择结束时间'}}</text>
					</view>
				</view>
				<view class="form-row">
					<view class="row-left">
					...
					<view class="row-input">
						...
						<text @click="showUnit = true"
							:class="{'text-c3':form.salary_unit == ''}">{{ form.salary_unit ? form.salary_unit : '请先选择单位，再输入薪资'}}</text>
					</view>
				...
				<view class="form-row column">
					...
					<view class="row-textarea">
						<u-textarea :maxlength="300" height="80" border="none" v-model="form.strength"
							:customStyle="customStyle1" placeholder="请输入项目需求" />
				...
				<view class="form-row column">
					...
					<view class="row-textarea">
						<u-textarea :maxlength="300" height="80" border="none" v-model="form.experience"
							:customStyle="customStyle1" placeholder="请输入岗位职责" />
		...
		<u-datetime-picker :show="showStartDate" confirmColor="#02AAAB" cancelColor="#B8B8B8" title="选择时间"
			:minDate="minDate" @confirm="confirmDate" @cancel="showStartDate = false" mode="date" />

		<u-datetime-picker :show="showEndDate" confirmColor="#02AAAB" cancelColor="#B8B8B8" title="选择时间"
			:minDate="minDate" @confirm="confirmEndDate" @cancel="showEndDate = false" mode="date" />
	...
	export default {
		...
		methods: {
			...
			upload_images() {
				...
				const uploadPromises = this.new_images.map((this_one) => {
					return new Promise((resolve, reject) => {
						upload_file("upload_project_image_to_OSS", this_one, 'this_one',
							"user", (res) => {
								let response;
								try {
			...
			async save_head_pic() {
				...
				let sign1 = encodeURIComponent('pathhead_picrealname' + _this.form.realname + sign_prefix + token +
					sign_prefix).replace(/!/g, '%21').replace(/'/g, '%27').replace(/\(/g, '%28').replace(/\)/g,
					'%29').replace(/\*/g, '%2A')
