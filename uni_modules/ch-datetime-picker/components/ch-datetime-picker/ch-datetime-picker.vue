<template>
	<view class="picker" :class="mask?'mask':''" @touchmove.stop.prevent>
		<view class="picker-wrap" :style="styles">
		
			<!-- 日期选择 -->
			<picker-view class="picker-wrap-dateTime" :indicator-style="indicatorStyle" v-if="params['y+']" indicator-class="picker-wrap-item"
				:value="dateIndexArr" @change="bindDateChange">
				<picker-view-column v-if="params['y+']" class="picker-wrap-column">
					<view class="picker-wrap-column-item" :class="{black:checkIndexRange[0]==index}" v-for="(item,index) in checkRange[0]" :key="index">
						{{item}}
					</view>
				</picker-view-column>
				<picker-view-column v-if="params['M+']" class="picker-wrap-column">
					<view class="picker-wrap-column-item" :class="{black:checkIndexRange[1]==index}" v-if="checkRange[1].length>0"
						v-for="(item,index) in checkRange[1]" :key="index">
						{{item<10?'0'+item:item}}
					</view>
				</picker-view-column>
				<picker-view-column v-if="params['d+']" class="picker-wrap-column">
					<view class="picker-wrap-column-item" :class="{black:checkIndexRange[2]==index}" v-for="(item,index) in checkRange[2]" :key="index">
						{{item<10?'0'+item:item}}
					</view>
				</picker-view-column>
			</picker-view>
		...
		initDateTime(date, minDate, maxDate, checkRange, checkIndexRange, idx = 0) {
			...
			for (let i = idx; i < 6; i++) {
				...
				if (i === 0) {
					...
					if (minFlag.reduce((cur, prev) => cur + prev, 0)) {
						t_begin = t_bData[i]
					} else {
						t_begin = t_minData[i]
					}
					if (maxFlag.reduce((cur, prev) => cur + prev, 0)) {
						t_end = t_eData[i]
					} else {
						t_end = t_maxData[i]
					}
	...
	export default {
		...
		methods: {
			show() {
				this.visible = true
				setTimeout(res=>{
					this.styles="transform:scale(1);opacity:1;"
				},200)
				this.init()
			},
			hide() {
				this.styles="transform:scale(1.2);opacity:0;"
				setTimeout(res=>{
					this.visible = false
				},200)
			},
			init() {
				...
				const res = DateTools.initDateTime(this.checkedValue, this.minDate, this.maxDate, this.checkRange, this
					.checkIndexRange)
			...
			bindDateChange(e) {
				...
				this.checkedValue = DateTools.mergeDateArr(this.checkRange, this.checkIndexRange)
				
				let datetime = DateTools.parse(this.checkedValue)
				let returnValue = DateTools.stringify(DateTools.parse(this.checkedValue), this.format)
				const array = {
			...
			bindTimeChange(e) {
				...
				t_value.forEach((item, index) => {
					if (item !== t_timeIndexArr[index]) {
						...
						// 因为微信小程序picker-view的异步加载问题,所以需要分离值小于等于30和大于30的赋值
						// #ifdef MP
						if (res.checkIndexRange[4] > 30 || res.checkIndexRange[5] > 30) {
							setTimeout(() => {
								this.checkIndexRange = res.checkIndexRange
								this.timeIndexArr = res.checkIndexRange.slice(3, 6)
							}, 0)
